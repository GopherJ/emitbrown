...
fn must_register_multiple_events_and_call_all_the_callback_passing_a_copy_of_the_same_data() {
    let (test, data_inside) = ("TEST".to_string(), "DATA INSIDE".to_string());
    let (mut emitter, callback, second_callback) = (
        Emitter::new(),
        Box::new(
            |event: &mut HashMap<String, String>| match event.get(&test) {
                Some(thing) => {
                    if *thing != data_inside {
                        panic!(
                        "the data inside the event is not what is expected, expected {}, get {}",
                        data_inside, *thing
                    );
                    }
                }
...
fn must_unregister_an_event() {
    ...
    let (mut emitter, callback) = (
        Emitter::new(),
        Box::new(|event: &mut HashMap<String, String>| {
            panic!(
                "the emitter must never be called instead has been called with {} parameters",
                event.capacity()
            );
        }),
    );
...
fn must_work_with_a_custom_event_data_and_allow_the_callback_to_modified_it() {
    ...
    let (mut emitter, callback) = (
        Emitter::new(),
        Box::new(|event: &mut HashMap<String, Foo>| {
